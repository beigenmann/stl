	...
	difference() {
		translate([0,(PSU_LENGTH / 2) + (PC_DEPTH  - PSU_INSET) / 2,0]) 
			difference() {
				...
				//Power Socket Hole
				translate([0.5*PSU_WIDTH + 1.51,10,0]) rotate([0,90,0]) powerSocket();
				
				//Power Switch Hole
				translate([0,PSU_INSET/2,(PSU_HEIGHT / 2) + 1]) rotate([0,0,90]) powerSwitch();
				
				//DC Wire Exit Hole
				translate([PSU_WIDTH/2 - 28.5,PC_DEPTH/2,PSU_HEIGHT / -2 + 10]) rotate([90,0,0]) cylinder(r=5,h=20, center=true);
			}	
		powerSupply();
	}
}
